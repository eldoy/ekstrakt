...
for (const fixture of fixtures) {
  test('extracts email ("' + fixture + '")', (t) => {
    t.is(extract.emails(fixture)[0], 'mail@example.com')
  })
}
...
test('not excludes emails with invalid TLD (mail@example.png)', (t) => {
  t.is(extract.emails('mail@example.png')[0], 'mail@example.png')
})
...
test('normalizes different email formats', (t) => {
  t.is(extract.emails('MAIL@EXAMPLE.COM')[0], 'mail@example.com')
  t.is(extract.emails('：mail@example.com')[0], 'mail@example.com')
  t.is(extract.emails('📧mail@example.com')[0], 'mail@example.com')
  t.is(extract.emails('m a i l [at] e x a m p l e [dot] c o m')[0], 'mail@example.com')
  t.is(extract.emails('foo m a i l [at] e x a m p l e [dot] c o m bar')[0], 'mail@example.com')
  t.is(extract.emails('mail[at]example[dot]co[dot]uk')[0], 'mail@example.co.uk')
  t.is(extract.emails('mail[at]example[dot]com')[0], 'mail@example.com')
  t.is(extract.emails('mail(at)example(dot)com')[0], 'mail@example.com')
  t.is(extract.emails('mail [at] example   [dot]   com')[0], 'mail@example.com')
  t.is(extract.emails('mail (at) example   (dot)   com')[0], 'mail@example.com')
  t.is(extract.emails('mail <at> example   <dot>   com')[0], 'mail@example.com')
